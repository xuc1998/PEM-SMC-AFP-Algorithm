...
2. **Adaptive Tempering (AT)** – chooses temperature steps so that rCESS stays inside
...
- `PATPEM_SMC.m` – orchestrates tempering, (re)weighting, resampling, and move operators (ARM / XOVER / DE–MH).  
...
opts.Tempering.MaxDeltaBeta = 0.2;    % optional hard cap
